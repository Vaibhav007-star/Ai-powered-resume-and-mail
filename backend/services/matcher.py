...
def evaluate_candidate_match(profile: Dict[str, Any], job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for p in profile.get("projects", []):
        tech_str = p.get("tech", "")
        for t in RE_SPLIT_TECH.split(tech_str):
            if t.strip():
    ...
    def candidate_has_skill(skill_name: str) -> bool:
        ...
        if s_lower in candidate_skills_set:
            return True
        if s_lower not in _skill_regex_cache:
            _skill_regex_cache[s_lower] = re.compile(r"\b" + re.escape(s_lower) + r"\b")
        return bool(_skill_regex_cache[s_lower].search(resume_text_lower))
    ...
    ats_info = analyze_ats_optimization(profile, job)
    semantic_fit = ats_info.get("semantic_similarity_score", 50)
    ...
    overview_explanation = (
        f"Overall match score is {final_score}% (Semantic Fit: {semantic_fit}%). "
    ...
    return {
        ...
        "semantic_similarity_score": semantic_fit,
        "education": {
            "status": edu_status,
            "required": req_degree,
