...
from backend.config import GROQ_API_KEY, GROK_API_KEY
...
def parse_profile_heuristically(raw_text: str) -> Dict[str, Any]:
    ...
    # 2. Phone extraction (common global/US formats)
    phone_match = re.search(r"(?:\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}", raw_text)
    phone = phone_match.group(0) if phone_match else ""
    
    # 3. Name heuristic (first non-empty line usually contains the candidate name)
...
def extract_structured_profile(raw_text: str) -> Dict[str, Any]:
    ...
    Enforces strict zero-hallucination constraint.
    """
